...
class curl {
    ...
    public function get_curl($url, $alter = false)
    {
		...
		if ($httpcode >= 200 && $httpcode < 300)
			...
			log_message_flex('error', 'CURL NULL: HTTP CODE: '.$httpcode.' - URL: '.$url.'', 'FATAL ERROR');
			if($alter == false){
				$new_url = str_replace(URL_AUTHENSERVER, URL_ALTER_AUTHEN, $url);
				log_message_flex('error', 'New URL: '.$new_url, 'FATAL ERROR');
				return $this->get_curl($new_url, true);
			}
			return false;
		}
    }
